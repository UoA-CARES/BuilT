...
class TrainerBase(object):
    def __init__(self, config, builder, wandb_run=None, wandb_conf=None, working_dir=None):
        self.config = config
        ...
        #     self.writer['wandb'] = self.wandb_run
    ...
    def process_single_epoch(self, dataloader: DataLoader, epoch: int, split, is_train: bool) -> float:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            for step, (inputs, targets) in tbar:
                ...
                if isinstance(loss, dict):
                    ...
                    loss_dict = {'loss': loss}
                self.backward(loss=loss, optimizer=self.optimizer, step=step, is_train=is_train)

                f_epoch = epoch + step / total_step
                ...
                tbar.set_description(f'{phase} {f_epoch: .2f} epoch')
                tbar.set_postfix(lr=lr, loss=f'{loss.item():.5f}')
            
            logger.aggregate()
            return logger.score

    def backward(self, loss, optimizer, step, is_train=True):
        if is_train:
            loss.backward()
            if self.config.train.gradient_accumulation_step is None:
                optimizer.step()
        ...
        for epoch in range(last_epoch, self.config.train.num_epochs):
            ...
            for d in self.dataloaders:
                ...
                if is_train:
                    ...
                    self.process_single_epoch(dataloader, epoch, split, is_train=is_train)
            ...
            for d in self.dataloaders:
                ...
                if not is_train:
                    dataloader = d['dataloader']
                    score = self.process_single_epoch(dataloader, epoch, split, is_train=is_train)
                    if split == 'test':
                        ckpt_score = score
                    else:
