...
DEVICE_ID=0
CLASSIFICATION_CONFIG=tweet/config/tweet_sentiment_classification_roberta_large.yaml
INDEX_EXTRACTION_CONFIG=tweet/config/tweet_index_extraction.yaml

[ ! -d "tweet/input" ] && cd tweet && sh download_data.sh && cd ..
[ ! -d "tweet/input/roberta-large" ] && cd tweet && sh download_model_roberta.sh && cd ..
